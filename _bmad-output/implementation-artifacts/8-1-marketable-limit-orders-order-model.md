...
Adversarial code review 2026-08-04 (Blind Hunter + Edge Case Hunter + Acceptance Auditor over `fec8ae7..f8b9d81`). Auditor confirmed all 5 ACs and both locked decisions structurally met; findings below are message/robustness gaps, not AC-logic failures.

- [x] [Review][Patch] Deferred-feature 422 delivers a GENERIC "Request validation failed", not AC-3's explicit "not supported in this version" [ballast/backend/api/coach.py:145-187, 632; ballast/backend/tests/test_coach_api.py:2064] — The `OrderIntentIn._validate_order_matrix` pydantic `model_validator` (`api/coach.py:145`) raises `ValueError` for stop/stop_limit/am/pm/gtc, but FastAPI wraps it in `RequestValidationError`, and the global handler at `api/app.py:134` returns a fixed `"Request validation failed"` — the calm copy is discarded. As a consequence the `except OrderNotSupportedError` arm in `approve` (`api/coach.py:632`) is DEAD on the HTTP path (the boundary rejects first, so the engine gate never sees a deferred intent). AC-2's non-marketable message DOES reach the user (adapter → `OrderNotPlaceableError` → `HTTPException(422, str(exc))`, test asserts "coming later" at `test_coach_api.py:2028`); only AC-3's deferred-feature message is swallowed. `test_approve_deferred_features_rejected_422_no_broker` (`:2064`) asserts only `status_code == 422` + `spy.calls == []`, never the message. **RESOLUTION (MasterB 2026-08-04): fix now via the engine gate.** Remove the deferred-feature (stop/stop_limit/am/pm/gtc) checks from `_validate_order_matrix` — keeping the market/limit price-shape checks — so the request reaches `approve`, where the existing `OrderNotSupportedError` arm returns the explicit `str(exc)` 422. Then strengthen `test_approve_deferred_features_rejected_422_no_broker` to assert the message contains "not supported in this version". Engine gate (`validate_order_intent`) stays the single authoritative gate (accepted trade-off vs Task 6's boundary layer).
- [x] [Review][Patch] Adapters lack a local guard on `limit_price` None/NaN/Inf [ballast/backend/brokers/fake_adapter.py:203; ballast/backend/brokers/schwab_adapter/adapter.py:390] — both LIMIT branches do `limit_price < reference` / `amount / limit_price` with no adapter-local finiteness/None guard, trusting the upstream `validate_order_intent` gate. Defense-in-depth only (the gated production path is safe); but a direct/future caller with `limit_price=None` → `TypeError`, and in the schwab branch the raw exception sits INSIDE the transport net → misclassified as INDETERMINATE/timeout rather than a calm refusal. Fix: raise `OrderNotPlaceableError` on a non-finite/None limit at the top of each LIMIT branch, mirroring the existing unusable-quote refusal.
- [x] [Review][Patch] SELL sub-share refusal message says "buys" [ballast/backend/brokers/fake_adapter.py:227; ballast/backend/brokers/schwab_adapter/adapter.py:405] — the sub-share guard hardcodes `"${amount} buys less than one whole share"` on both sides; a SELL LIMIT that floors to <1 share tells the seller their order "buys". Cosmetic copy defect in the calm-refusal path (no AC asserts the sub-share text). Fix: side-aware verb.
- [x] [Review][Patch] `OrderIntentOut` emits `null` keys on the wire, contradicting its "mirrors the persisted snapshot" docstring [ballast/backend/api/coach.py:203-207, 356] — `_order_intent_out` sets defaulted fields to `None`, but no `response_model_exclude_none=True` is set, so a MARKET intent serializes as `{symbol, side, amount, order_type:null, limit_price:null, ...}` on `/recommend`, NOT the `{symbol, side, amount}` the docstring claims. Additive nulls only (the Coach never proposes a limit, so it is always the MARKET case); no functional harm. Fix: either add `response_model_exclude_none=True` to the recommend/replay responses or correct the docstring to say "null-when-default on the wire".
- [x] [Review][Defer] No tick-size / penny quantization of `limit_price` before Schwab [ballast/backend/brokers/schwab_adapter/adapter.py:430] — deferred, forward-looking. `format_money(limit_price)` passes the user's precision verbatim; only `> 0`/finite is validated (the AC-3 matrix requires nothing more). A sub-penny limit (e.g. `"100.999"`) would be sent as-is and REJECTED by a live broker. Not reachable in Story A (no UI, LLM never sets a price); belongs to the Story C order-entry UI validation and/or the 7.7 real-money gate.

Dismissed as noise (2): SELL sizing on the limit-floor price (consistent with the pre-existing market-path `amount/price` sizing convention; fill-at-limit keeps dollar sizing coherent — not an AC violation); `session`/`duration` not propagated to the schwab builder (the builder's DAY/NORMAL defaults match the ONLY reachable values since am/pm/gtc are gated upstream — correct today, Story B territory).
