...
- **D3 — Defer to the deploy plan.** When a BUY toward bonds already answers the shortfall (investable cash present and the deploy path would buy bonds), this check DEFERS — no double-spend of the same cash; it only emits a SELL-to-bonds finding when cash is insufficient.
- **D4 — `BOND_SHORTFALL = Decimal("0.15")`** (15 percentage points). Fires only when `target_bond_pct − current_bond_pct > BOND_SHORTFALL`.
